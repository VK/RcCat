...
#define getRegister(registerAddr) i2c.readByteLongAddr(address, registerAddr)
#define getRegister16bit(registerAddr)                                         \
  i2c.read2BytesLongAddr(address, registerAddr)
#define setRegister(registerAddr, data)                                        \
  i2c.writeByteLongAddr(address, registerAddr, data)
#define setRegister16bit(registerAddr, data)                                   \
  i2c.write2BytesLongAddr(address, registerAddr, data)
...
uint8_t RangeFinder::internalSetup() {
  ...
  setRegister(VL6180X_SYSTEM_INTERRUPT_CONFIG_GPIO,
              (4 << 3) | (4)); // Set GPIO1 high when sample complete

  setRegister(VL6180X_SYSTEM_MODE_GPIO1,
              0x10); // Set GPIO1 high when sample complete
  setRegister(VL6180X_READOUT_AVERAGING_SAMPLE_PERIOD,
              0x30);                               // Set Avg sample period
  setRegister(VL6180X_SYSALS_ANALOGUE_GAIN, 0x46); // Set the ALS gain
  setRegister(VL6180X_SYSRANGE_VHV_REPEAT_RATE,
              0xFF); // Set auto calibration period (Max = 255)/(OFF = 0)
  setRegister(VL6180X_SYSALS_INTEGRATION_PERIOD,
              0x63); // Set ALS integration time to 100ms
  setRegister(VL6180X_SYSRANGE_VHV_RECALIBRATE,
...
void RangeFinder::setup(int in_address) {
  address = in_address; // set default address for communication
...
uint8_t RangeFinder::getDistance() { return distance; }
