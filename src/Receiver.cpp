...
namespace RcCat
{
  ...
  void Receiver::attach(int pinID, volatile void (*f)(int microseconds))
  {
    ...
    *p_PCICR |= _BV(digitalPinToPCICRbit(pinID));
    ...
    p_input_pin_register[registered_channels] = portInputRegister(digitalPinToPort(pinID));
    ...
    Serial.print("register Channel: "); Serial.println(registered_channels);
    Serial.print("id: "); Serial.println(input_pin_bitMask[registered_channels]);
  ...
  void Receiver::pinChangeIntISR()
  {
    ...
    speed_pin_state_new = *speed_p_input_pin_register & speed_input_pin_bitMask ? HIGH : LOW;
    ...
    if (speed_pin_state_old != speed_pin_state_new)
    {
      ...
      if (speed_pin_state_new == HIGH)
      {
        ...
        speed_pd = speed_t_start - speed_t_start_old; //0.5us units, the incoming pulse period
        dspeed = -speed;
    ...
    for(int pinNr = 0; pinNr < registered_channels; pinNr++)
    {
      ...
      pin_state_new[pinNr] = *p_input_pin_register[pinNr] & input_pin_bitMask[pinNr] ? HIGH : LOW;
      ...
      if (pin_state_old[pinNr] != pin_state_new[pinNr])
      {
        ...
        if (pin_state_new[pinNr] == HIGH)
        {
          ...
          pd[pinNr] = t_start[pinNr] - t_start_old[pinNr]; //0.5us units, the incoming pulse period
  ...
  float Receiver::getSpeed()
  {
    return speed;
  }

  float Receiver::getDSpeed()
  {
    return dspeed;
  }
