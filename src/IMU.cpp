...
IMU::IMU() { filter = ComplementaryFilter(); }
...
void IMU::setup(uint8_t gscale, uint8_t ascale) {
  ...
  /*
    if (c == 0x71) // WHO_AM_I should always be 0x68
    {
      ...
      runSelfTest(SelfTest); // Start by performing self test and reporting
    values Serial.print("x-axis self test: acceleration trim within : ");
      ...
      runCalibration(gyroBias, accelBias);
      Serial.print("x-axis acceleration bias: ");
      Serial.println(accelBias[0]);
      Serial.print("y-axis acceleration bias: ");
      Serial.println(accelBias[1]);
      Serial.print("z-axis acceleration bias: ");
      Serial.println(accelBias[2]);
      Serial.print("x-axis gyro bias: ");
      Serial.println(gyroBias[0]);
      Serial.print("y-axis gyro bias: ");
      Serial.println(gyroBias[1]);
      Serial.print("z-axis gyro bias: ");
...
void IMU::runSelfTest(float *destination) // Should return percent deviation
from factory trim values, +/- 14 or less deviation is a pass
{
  ...
  i2c.writeByte(MPU9250_ADDRESS, SMPLRT_DIV, 0x00);      // Set gyro sample rate
to 1 kHz i2c.writeByte(MPU9250_ADDRESS, CONFIG, 0x02);          // Set gyro
sample rate to 1 kHz and DLPF to 92 Hz i2c.writeByte(MPU9250_ADDRESS,
GYRO_CONFIG, FS << 3);  // Set full scale range for the gyro to 250 dps
  i2c.writeByte(MPU9250_ADDRESS, ACCEL_CONFIG2, 0x02);   // Set accelerometer
rate to 1 kHz and bandwidth to 92 Hz i2c.writeByte(MPU9250_ADDRESS,
ACCEL_CONFIG, FS << 3); // Set full scale range for the accelerometer to 2 g

  for (int ii = 0; ii < 200; ii++)
  { // get average current values of gyro and acclerometer

    i2c.readBytes(MPU9250_ADDRESS, ACCEL_XOUT_H, 6, &rawData[0]);  // Read the
six raw data registers into data array aAvg[0] += (int16_t)(((int16_t)rawData[0]
<< 8) | rawData[1]); // Turn the MSB and LSB into a signed 16-bit value aAvg[1]
+= (int16_t)(((int16_t)rawData[2] << 8) | rawData[3]); aAvg[2] +=
(int16_t)(((int16_t)rawData[4] << 8) | rawData[5]);

    i2c.readBytes(MPU9250_ADDRESS, GYRO_XOUT_H, 6, &rawData[0]);   // Read the
six raw data registers sequentially into data array gAvg[0] +=
(int16_t)(((int16_t)rawData[0] << 8) | rawData[1]); // Turn the MSB and LSB into
a signed 16-bit value gAvg[1] += (int16_t)(((int16_t)rawData[2] << 8) |
rawData[3]); gAvg[2] += (int16_t)(((int16_t)rawData[4] << 8) | rawData[5]);
  }
  ...
  i2c.writeByte(MPU9250_ADDRESS, ACCEL_CONFIG, 0xE0); // Enable self test on all
three axes and set accelerometer range to +/- 2 g i2c.writeByte(MPU9250_ADDRESS,
GYRO_CONFIG, 0xE0);  // Enable self test on all three axes and set gyro range to
+/- 250 degrees/s delay(25);                                          // Delay a
while to let the device stabilize

  for (int ii = 0; ii < 200; ii++)
  { // get average self-test values of gyro and acclerometer

    i2c.readBytes(MPU9250_ADDRESS, ACCEL_XOUT_H, 6, &rawData[0]);    // Read the
six raw data registers into data array aSTAvg[0] +=
(int16_t)(((int16_t)rawData[0] << 8) | rawData[1]); // Turn the MSB and LSB into
a signed 16-bit value aSTAvg[1] += (int16_t)(((int16_t)rawData[2] << 8) |
rawData[3]); aSTAvg[2] += (int16_t)(((int16_t)rawData[4] << 8) | rawData[5]);

    i2c.readBytes(MPU9250_ADDRESS, GYRO_XOUT_H, 6, &rawData[0]);     // Read the
six raw data registers sequentially into data array gSTAvg[0] +=
(int16_t)(((int16_t)rawData[0] << 8) | rawData[1]); // Turn the MSB and LSB into
a signed 16-bit value gSTAvg[1] += (int16_t)(((int16_t)rawData[2] << 8) |
rawData[3]); gSTAvg[2] += (int16_t)(((int16_t)rawData[4] << 8) | rawData[5]);
  }
  ...
  selfTest[0] = i2c.readByte(MPU9250_ADDRESS, SELF_TEST_X_ACCEL); // X-axis
accel self-test results selfTest[1] = i2c.readByte(MPU9250_ADDRESS,
SELF_TEST_Y_ACCEL); // Y-axis accel self-test results selfTest[2] =
i2c.readByte(MPU9250_ADDRESS, SELF_TEST_Z_ACCEL); // Z-axis accel self-test
results selfTest[3] = i2c.readByte(MPU9250_ADDRESS, SELF_TEST_X_GYRO);  //
X-axis gyro self-test results selfTest[4] = i2c.readByte(MPU9250_ADDRESS,
SELF_TEST_Y_GYRO);  // Y-axis gyro self-test results selfTest[5] =
i2c.readByte(MPU9250_ADDRESS, SELF_TEST_Z_GYRO);  // Z-axis gyro self-test
results

  // Retrieve factory self-test value from self-test code reads
  factoryTrim[0] = (float)(2620 / 1 << FS) * (pow(1.01, ((float)selfTest[0]
- 1.0))); // FT[Xa] factory trim calculation factoryTrim[1] = (float)(2620 / 1
<< FS) * (pow(1.01, ((float)selfTest[1] - 1.0))); // FT[Ya] factory trim
calculation factoryTrim[2] = (float)(2620 / 1 << FS) * (pow(1.01,
((float)selfTest[2] - 1.0))); // FT[Za] factory trim calculation factoryTrim[3]
= (float)(2620 / 1 << FS) * (pow(1.01, ((float)selfTest[3] - 1.0))); // FT[Xg]
factory trim calculation factoryTrim[4] = (float)(2620 / 1 << FS) * (pow(1.01,
((float)selfTest[4] - 1.0))); // FT[Yg] factory trim calculation factoryTrim[5]
= (float)(2620 / 1 << FS) * (pow(1.01, ((float)selfTest[5] - 1.0))); // FT[Zg]
...
100.;         // Report percent differences destination[i + 3] = 100.0 *
((float)(gSTAvg[i] - gAvg[i])) / factoryTrim[i + 3] - 100.; // Report percent
differences
  }
}
*/
...
void IMU::runCalibration(float *dest1, float *dest2) {
  ...
  // reset device
  i2c.writeByte(MPU9250_ADDRESS, PWR_MGMT_1,
  ...
  i2c.writeByte(MPU9250_ADDRESS, USER_CTRL,
  ...
  i2c.writeByte(MPU9250_ADDRESS, GYRO_CONFIG,
  ...
  // Configure FIFO to capture accelerometer and gyro data i2cfor bias
  // calculation
  i2c.writeByte(MPU9250_ADDRESS, USER_CTRL, 0x40); // Enable FIFO
  i2c.writeByte(MPU9250_ADDRESS, FIFO_EN,
  ...
  i2c.readBytes(MPU9250_ADDRESS, FIFO_COUNTH, 2,
  ...
  for (ii = 0; ii < packet_count; ii++) {
    ...
    i2c.readBytes(MPU9250_ADDRESS, FIFO_R_W, 12,
  ...
  i2c.readBytes(MPU9250_ADDRESS, XA_OFFSET_H, 2,
...
void IMU::runInit() {
  ...
  uint8_t c = i2c.readByte(
      MPU9250_ADDRESS, GYRO_CONFIG); // get current GYRO_CONFIG register value
  ...
  c = i2c.readByte(MPU9250_ADDRESS,
                   ACCEL_CONFIG); // get current ACCEL_CONFIG register value
  // c = c & ~0xE0; // Clear self-test bits [7:5]
  c = c & ~0x18;       // Clear AFS bits [4:3]
  c = c | Ascale << 3; // Set full scale range for the accelerometer
  i2c.writeByte(MPU9250_ADDRESS, ACCEL_CONFIG,
  ...
  i2c.writeByte(MPU9250_ADDRESS, ACCEL_CONFIG2,
  ...
  i2c.writeByte(MPU9250_ADDRESS, INT_ENABLE,
...
bool IMU::loop() {
  ...
  if (i2c.readByte(MPU9250_ADDRESS, INT_STATUS) &
    ...
    i2c.readBytes(MPU9250_ADDRESS, GYRO_XOUT_H, 6,
...
float IMU::getAtot() { return a_tot; }

float IMU::getAx() { return a_x; }

float IMU::getAy() { return a_y; }

float IMU::getAz() { return a_z; }
